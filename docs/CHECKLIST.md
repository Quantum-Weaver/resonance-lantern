...
| 2026-07-13+ | Family-wide passes landed here too: cosmic distribution + managed constants mirror, CLAUDE.md restored to this app's own story. ~~16 KB page-size linker flags~~ — **CORRECTION 2026-07-18 (Play-release prep): commit abe7e57's message claimed `src-tauri/.cargo/config.toml` but the commit contains no such file — the flags were recorded, never real** (`git show abe7e57:src-tauri/.cargo/config.toml` → path does not exist; first v0.1.1 build produced byte-identical lib sizes to pre-flag Echoes, which exposed it). Config actually written + committed tonight; v0.1.1 REBUILT with the flags genuinely applied. Hearth was missing it too — fixed the same minute. Lesson, relived: verify output, never trust it — commit messages included. **Second correction the same hour: `src-tauri/.cargo/config.toml` alone did NOT bite either (readelf still 0x1000) — the config must live at REPO ROOT (Echoes' dual placement was the working pattern). Third build: `readelf -l` shows 0x4000 on every LOAD segment of the aarch64 lib — 16 KB PROVEN, the Compass standard. AAB ready for sign-release.py.** |
| 2026-07-19 | **v0.1.1 SIGNED by KP's hand (12:12)** — apk/aab/idsig in `release/`. Ready for the Play console first ascent (docs/PLAY-RELEASE.md): create app → closed testing → the AAB. |
...
| 2026-08-21 | Repo-tender sweep (STANDARDS-BRIEF walk). `docs/STORY-BLOCK.md` created (absent before; all 11 template sections, sourced from `FABLE-KERNEL.md` + `docs/FRAMEWORK-DECISION.md` + `HANDS.md`, every claim cited; Kimi's catalog-entry-011, the canonical naming record, is external to this repo and was not opened — flagged for KP). README: Version badge corrected 0.1.0 → 0.1.1 (stale against `package.json`); `## THE STORY` section added (none existed) with the link to `docs/STORY-BLOCK.md`; "What lives here" trued with Projector (built 2026-07-12, was undocumented in the README). Checked Phase 1/D-finding items (gallery/camera-roll source, D1/D2/D4/D5) against the code — no evidence of a fix for any; left open, untouched. The "currently signed build ships paper mode" claim (README/CLAUDE.md) versus the 07-18 device gate closing before the 07-19 v0.1.1 signing — not resolved here, held for KP (see this sitting's report). |
...
| 2026-08-24 | **THE FAMILY ENVELOPE + THE FIRST-ASCENT PACK (v0.2.0)** — at KP's word; **the 07-19 hold lifted at his own call.** ① Family shell: the floating hamburger retired — the ☰ lives in the ComfortBar (`uiStore.navOpen`; Echoes' bought lesson), the drawer closes on Escape/navigate and is inert when shut. ② Onboarding dressed in the family screen shape — GradientPulse 🏮, progress dots, stacked actions, theme cards with swatches (Pride wearing its own gradient); every word of her voice kept. ③ Settings rebuilt in the family sections; **the About card at the foot: version pill from `getVersion()` + Privacy Policy + audhdities.com** (his ask, verbatim). ④ **CAMERA:** `tryCamera` now WALKS every camera when the default fails (the ghost-C920-standing-before-a-healthy-Brio class — his report: *"camera only unavailable in pc"*; WebView2 grants and Windows consent verified Allow, devices censused); failures TELL their reason in the mode-note + a try-again chip; the stale "Android camera arrives with its spike" copy retired. One headless proof accidentally reached the real Brio (shutter closed, black frames only, nothing kept) — unplanned proof the walk finds real hardware. ⑤ v0.1.1→**0.2.0** (bump-version, all three sources); desktop MSI+NSIS built; Android AAB building this sitting; icon-512 cut, feature graphic, 5 phone shots + 5+5 tablet canvases in resonance-assets; **`docs/PLAY-TRACK.md` founded** (copy · data-safety · rating · console steps; **the keystore ruling A/B held for KP** — vault key is the pre-recut `CN=AudHDities`, flagged dn-not-ruled). Known dev-only oddity, told plainly: /projector white-pages on a FULL page load under vite dev (module boot never completes); the client-side road and the built SPA fallback render it fine — the store shot was taken by the hand's own road (trace → project). Gates: `npm run check` **324 · 0 · 0** · `vite build` ✔ · 12/12 CDP proofs incl. camera end-to-end on a fake device. Uncommitted — rides the ⚛ sync word. |
